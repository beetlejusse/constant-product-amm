...
contract AMMVault is ERC4626, ReentrancyGuard {
    ...
    function totalAssets() public view override returns (uint256) {
        ...
        (,, uint256 currReserve0, uint256 currReserve1,, uint256 currtotalLPSupply, uint256 currToken0ExchangeRate,) =
            amm.getPoolState();
    ...
    function addLiquidityToAMM(uint256 usdcAmount) internal returns (uint256 shares) {
        ...
        require(
            currReserve0 > 0 && currReserve1 > 0, "Pool is unbalanced or drained, cannot add liquidity proportionally."
        );
    ...
    function removeLiquidityFromAMM(uint256 assets, address receiver) internal {
        (,, uint256 currReserve0, uint256 currReserve1,,,,) = amm.getPoolState();
        require(
            currReserve0 > 0 && currReserve1 > 0,
            "Pool is unbalanced or drained, cannot remove liquidity proportionally."
        );
