...
contract ConstantProductAMM is ReentrancyGuard {
    ...
    function addLiquidityToPool(uint256 _reserveAdded0, uint256 _reserveAdded1) external nonReentrant returns(uint256 shares) {
    ...
    function removeLiquidityfromPool(uint256 _shares) external nonReentrant returns(uint256 reserveAmountRemoved0, uint256 reserveAmountRemoved1) {
    ...
    function getpoolState() external view returns(
        ...
        uint256 reserve_0, uint256 reserve_1, uint256 ratio,
        uint256 totalLPSupply, uint256 token0ExchangeRate, uint256 token1ExchangeRate 
    ) {
